...
cost=[]
for i in range(nodes):
	cost.append(random.randrange(5,10))
...
agent=1
select=[]
random_plot=open("random_data.dat","w+")

while temp < 100:
	random_node=random.randrange(0,15)
	while random_node in select:
		random_node=random.randrange(0,15)
	select.append(random_node)
	temp=temp+cost[random_node]
	if temp < 100:
		random_plot.write("%s %s\n" %(agent,(temp)))
	agent=agent+1
...
agent=1
select2=[]
random_plot2=open("random_data2.dat","w+")

while temp < 50:
	random_node=random.randrange(0,15)
	while random_node in select2:
		random_node=random.randrange(0,15)
	select2.append(random_node)
	temp=temp+cost[random_node]
	if temp < 50:
		random_plot2.write("%s %s\n" %(agent,(temp)))
	agent=agent+1
